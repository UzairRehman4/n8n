...
const { fakeLicenseServer, FakeLicenseManager } = vi.hoisted(() => {
	...
	/** Models SDK 3.2.0 `renewIfDue` over the store callbacks: re-read a changed store, then renew or skip. */
	class FakeLicenseManager {
		private cert: FakeCert | undefined;

		private loadedCertStr = '';
		...
		async renewIfDue(): Promise<void> {
			...
			if (await this.hasStoredCertChanged()) {
				await this.initCert();
				if (!this.isRenewalDue()) return;
			}
		...
		private async hasStoredCertChanged(): Promise<boolean> {
			return (await this.config.loadCertStr()) !== this.loadedCertStr;
		}

		private async initCert(): Promise<void> {
			const stored = await this.config.loadCertStr();
			this.loadedCertStr = stored;
 ...
 * the SDK is a fake with the 3.2.0 renewal contract and the server is in memory.
 */
describe('license renewal across two mains over one database', () => {
	...
	it('runs the pass on a main with a stale cert without sending the stale token', async () => {
		const job = await provisionJob(a.task);
		await a.license.renewIfDue();
		expect(fakeLicenseServer).toMatchObject({ requests: 1, rotations: 1 });
		expect(b.license.getExpiryDate()).toEqual(certExpiry(initialCert));
		const occurrence = await seedDueOccurrence(job);

		expect(await b.scheduler.execute()).toHaveLength(1);

		await retryUntil(async () => expect(await statusOf(occurrence)).toBe('succeeded'));
		expect(fakeLicenseServer).toMatchObject({ requests: 1, rejected: 0 });
		expect(b.license.getExpiryDate()).toEqual(a.license.getExpiryDate());
	...
	it('dead-letters a pass stranded by a dead main instead of renewing again', async () => {
		const job = await provisionJob(a.task);
		...
		expect(await b.scheduler.reap()).toEqual({ reclaimed: 0, deadLettered: 1, missed: 0 });

		const row = await taskRepo.findOneByOrFail({ id: stranded.id });
		expect(row).toMatchObject({ status: 'failed', attempts: 1 });
		expect(row.errorMessage).toMatch(/lease expired/i);
		expect(await a.scheduler.execute()).toEqual([]);
		expect(await b.scheduler.execute()).toEqual([]);
		expect(fakeLicenseServer.requests).toBe(0);
		expect(await a.license.loadCertStr()).toBe(initialCert);
	});
});
